<template>
  <v-layout>
    ...
    <v-app-bar border="b" class="ps-4" flat>
      ...
      <v-app-bar-title>Outbound Management</v-app-bar-title>
      ...
      <div class="pa-4">
        ...
        >
					<v-form v-model="valid" @keyup.enter="addProduct">
						...
						<v-text-field
							v-model="newProduct.remarks"
							label="Remarks"
							required
						></v-text-field>

						<v-text-field
							v-model="newProduct.destination"
							label="Destination"
							required
						></v-text-field>
            ...
            :items="products"
					...
					>
						...
						<template v-slot:item.remarks="{ item }">
							<template v-if="item.isEditing">
								<v-text-field v-model="item.remarks" density="compact" @keyup.enter="editProduct(item)"></v-text-field>
							</template>
							<template v-else>
								{{ item.remarks }}
							</template>
						...
						<template v-slot:item.destination="{ item }">
							<template v-if="item.isEditing">
								<v-text-field v-model="item.destination" density="compact" @keyup.enter="editProduct(item)"></v-text-field>
							</template>
							<template v-else>
								{{ item.destination }}
							</template>
...
<script>
	...
	export default {
		...
		methods: {
			addProduct() {
				if (this.newProduct.name && this.newProduct.remarks && this.newProduct.destination && this.newProduct.quantity >= 0) {
					...
					this.newProduct.remarks = '';
					this.newProduct.destination = '';
      ...
      editProduct(item) {
			...
			async deleteProduct(item) {
				...
				if (res) {
					...
					try {
						const	response = await axios.delete(`${API_BASE_URL}/outbound-products/${item.id}`);
			...
			async putProduct(item) {
				try {
					const response = await axios.put(`${API_BASE_URL}/outbound-products/${item.id}`, item);
					if (response.status !== 200)
          ...
          console.error("Error updating product:", error.response? error.response.data : error.message);
			...
			async fetchProducts() {
				try {
					const response = await axios.get(`${API_BASE_URL}/outbound-products`);
          ...
          console.error("Error fetching products ley:", error);
			...
			async postProducts(newProduct) {
				try {
					await axios.post(`${API_BASE_URL}/outbound-products`, newProduct);
				}
				catch (error) {
          console.error("Error posting product ley:", error);
			...
			async done(item) {
				const res = await this.confirm({ content: `${item.name} has arrived at ${item.destination}?`, title: 'Mark as done' });
				...
				if (res) {
					try {
						...
						console.error("Error checkmarking the outbound product:", error);
						if (error.response) {
							const statusCode = error.response.status;
							if (statusCode === 400)
								this.$toast.error("Invalid data. Please check your input");
